//+------------------------------------------------------------------+
//|                                       Signal To Forex Client.mq4 |
//|                                   Copyright 2013, UpDown Signals |
//+------------------------------------------------------------------+
#property copyright "Copyright 2013, UpDown Signals"
...
int ReadSuffixWaitingTime = 0;

string Signals[100];
int SignalIdx;

string GenericComment = "UpDown Signals";
...
int init()
{
   ...
   double SymPoints = MarketInfo(symbol, MODE_POINT);
   double SymDigits = MarketInfo(symbol, MODE_DIGITS);
...
int start()
{
   while(!IsStopped() && IsExpertEnabled() && IsConnected())
   {
      if(!SuffixesRead)
      {
         if (TimeLocal() >= ReadSuffixWaitingTime)
         {
            ReadSuffixes();
            ReadSuffixWaitingTime = TimeLocal() + 1;
         }
      }
      ReceiveSignals();
      Sleep(100);
...
void split(string& arr[], string str, string symbol)
{
   ArrayResize(arr, 0);
   string item;
   int pos, size;
...
double CalculateLotsize(string sym, double riskPercent, double stoploss)
{
   ...
   Print("Balance = " + bal + " | dollarsToRisk = " + dollarsToRisk + " | dollarsPerTick = " + 
         dollarsPerTick + " | newLotSize = " + newLotSize);
             ...
             
            {
               ...
               for (int i=0; i<10; i++)
               {
                  ...
                  {  
                          ...
                          
                     {
                        Print("Sending order with the following parameters: Symbol = " + symbol +
                                                                        ...
                                                                        " | Magic Number = " + ticket);
                        tk = OrderSend(symbol, OP_BUY, lots, MarketInfo(symbol, MODE_ASK), Slippage * PointFactor, 0, 0, GenericComment, ticket);
                        if(sl == 0)
                        {
                  ...
                  else if (type == OP_SELL)
                  {
                     ...
                     {
                        tk = OrderSend(symbol, OP_SELL, lots, MarketInfo(symbol,MODE_BID), Slippage*PointFactor, 0, 0, GenericComment, ticket); 
                        if(sl == 0)
                        {
                  ...
                  {             
                     ...
                     {                             
                        tk = OrderSend(symbol, type, lots, NormalizeDouble(price,digits), Slippage*PointFactor, 0, 0, GenericComment, ticket);   
                     }                        
                  }
